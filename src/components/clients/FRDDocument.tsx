...
import { 
  ...
  CalendarIcon, Plus, X, CheckCircle2, ClipboardList, ChevronDown
} from "lucide-react";
...
export function FRDDocument({ client }: FRDDocumentProps) {
  ...
  const [tasks, setTasks] = useState<Task[]>([
    { id: "1", title: "Review investment portfolio", actionDate: new Date(2024, 8, 15), status: 'pending', agendaItemId: "2", vertical: "Financial Planning" },
    { id: "2", title: "Update insurance coverage", actionDate: new Date(2024, 8, 20), status: 'pending', agendaItemId: "3", vertical: "Insurance" }
  ]);
          ...
          <Card className="flex-1 min-h-0 flex items-center justify-center">
            <div className="text-center p-6">
              ...
              <div className="space-y-4">
                <div>
                  <div className="text-3xl font-bold">{cumulativeStats.totalCalls}</div>
                  <p className="text-sm text-muted-foreground">Calls Completed</p>
                </div>
                <div>
                  <div className="text-xl font-semibold">{format(cumulativeStats.lastCallDate, "dd MMM yyyy")}</div>
                  <p className="text-sm text-muted-foreground">Last Call Date</p>
                </div>
                <div>
                  <div className={`text-xl font-semibold ${cumulativeStats.daysSinceLastCall > 30 ? "text-red-600" : ""}`}>
                    {cumulativeStats.daysSinceLastCall} days
                  </div>
